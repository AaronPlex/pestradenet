...
```{r}
host.distribution<-read.csv("FAOSTAT-Tomato-harveste-area-2011-2020.csv")
...
PestDistribution <- read.csv("C:/Users/plexaaron/UFL Dropbox/Aaron Plex Sula/168/2022/2022 Projects/Pathogen biogeography/Pathogen richness and diversity/Tomato/Tuta absoluta.csv")
PestDistribution<-PestDistribution[,c("Region","Presence", "Extent", "First.record")]
colnames(PestDistribution)<-c("ExportCountry","Presence", "Extent", "First.record")
...
PestDistribution$Biosecurity<-ifelse(PestDistribution$USOrder=="yes",2,1)
...
##Creating a matrix of host availability
#biomatr = TradingCountries$Biosecurity # yest = 10, no = 1
#biomatr = 1 / biomatr
...
#Geographic trade risk for pathogen introduction
...
## Node in-strength as a measure of pest introduction risk
...
strength.net <- ggraph(pestMoveNet, layout = "centrality", cent = country.strength^(1/4))+
  ...
  scale_fill_manual(values = c("Unreported" = "#FCB519FF", "Transient under eradication" = "#F98C0AFF", "Few occurrences" = "#E35932FF", "Localized" = "#BB3754FF", "Present" = "#89226AFF", "Widespread" =  "#56106EFF"))+
  scale_color_manual(values = c("Unreported" = "#FCB519FF", "Transient under eradication" = "#F98C0AFF", "Few occurrences" = "#E35932FF", "Localized" = "#BB3754FF", "Present" = "#89226AFF", "Widespread" =  "#56106EFF"))+
  coord_fixed()+
  theme_graph()+
...
strength.net + 
  guides(fill = guide_legend(title = "Reported distribution: \nTuta absoluta",
         ...
         color = guide_legend(title = "Reported distribution: \nTuta absoluta",
                             ...
                             title.theme = element_text(size = 15), 
                             label.theme = element_text(size = 15)),
            size = guide_legend(title = "Harvested \ntomato \nlog10(acreage)", 
                                title.theme = element_text(size = 15), 
                                label.theme = element_text(size = 15)), 
...
country.betweenness<-igraph::betweenness(pestMoveNet,v=V(pestMoveNet), directed = TRUE, weights = 1/E(pestMoveNet)$GeoTradeRisk)

country.PageRank <- page_rank(pestMoveNet, vids = V(pestMoveNet), directed = TRUE, weights = E(pestMoveNet)$GeoTradeRisk)

country.eigenvector <- eigen_centrality(pestMoveNet, directed = TRUE, weights = E(pestMoveNet)$GeoTradeRisk)

country.centralities<-data.frame(country.strength, country.betweenness, 
                                 country.eigenvector$vector, country.PageRank$vector)
country.centralities<-country.centralities%>%
  mutate(country.strength=country.strength/max(country.strength),
         ...
         country.PageRank.vector=country.PageRank.vector/max(country.PageRank.vector),
         PTRI = country.strength/3+country.betweenness/3+country.eigenvector.vector/6+country.PageRank.vector/6,
         PTRI = PTRI / max(PTRI))
```
